...
export const CalendarPage: React.FC = () => {
  ...
  // Fetch tasks for current project (if any)
  useEffect(() => {
    if (projects.length > 0) {
      // Fetch tasks for the first project to populate the task list
  ...
  const allTasks = useMemo((): CalendarTask[] => {
    ...
    // tasks is a flat array, filter by projectId and map to CalendarTask
    tasks.forEach(task => {
      if (task.dueDate) {
