...
export const useChat = () => {
  ...
  const createRoom = useCallback((data: { name: string; projectId?: string; participantIds: string[]; isGroup?: boolean }) => {
  ...
  const clearError = useCallback(() => {
    dispatch(clearChatOperationError());
  }, [dispatch]);
